                ...
                sections?.map((item, idx) => (
                    <div key={idx} className='mb-20 relative last:mb-0'>
                        <div className='max-w-[910px] mx-auto px-3 z-10 relative'>
                            <div className={`flex gap-5 lg:gap-0 ${item.direction === "right" ? "flex-col md:flex-row-reverse" : "flex-col md:flex-row"}`}>
                                <figure className={`md:w-[50%] ${item.direction === "right" ? "flex md:justify-end" : ""}`}>
                                    <Image src={`/images/left-right.jpg`} alt="" width={362} height={362} />
                                </figure>
                                <div className='flex-1'>
                                    <h4 className='!font-bold text-2xl md:text-[30px] font_montserrat'>{item?.title}</h4>
                                    <p className='text-xl font-medium mt-2 font_aleo mb-8 '>{item?.description}</p>
                                    <Link href={item?.buttonUrl} className='uppercase bg-[--brand-blue] border-[--brand-blue] text-white py-[14px] hover:border-black hover:text-black hover:bg-transparent border px-6 font-semibold rounded-3xl'>Get Started</Link>
                                </div>
                            </div>
                        </div>
                        <Image src={`/images/dots.jpg`} alt='' width={400} height={400} className={`absolute -top-5 opacity-40 ${item.direction === "right" ? "right-0" : "left-0"}`}/>
                    </div>
                ))
            }
        </sections>
