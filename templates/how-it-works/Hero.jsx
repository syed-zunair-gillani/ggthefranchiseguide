    ...
    <main className='min-h-screen w-full bg-cover bg-no-repeat flex justify-center relative' style={{
        ...
        <div className='!max-w-[1280px] flex flex-col sm:flex-row justify-center gap-5 sm:gap-10 items-center mx-auto px-3'>
            ...
            <div className='md:w-[50%] w-full py-20 '>
                ...
                <p className='font_roboto mb-10 mt-4 text-[26px]'>{data?.caption}</p>
                <Link href={data?.buttonLinks} className="uppercase bg-[--brand-blue] border-[--brand-blue] text-white py-[14px] hover:border-black hover:text-black hover:bg-transparent border px-6 font-semibold rounded-3xl">Get Started</Link>
            </div>
            <div className='flex justify-center w-[40%]'>
                <Image src={data?.image?.node?.mediaItemUrl} alt="" width={300} height={375}/>
            </div>
        </div>
